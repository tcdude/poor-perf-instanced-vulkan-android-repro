...
static void update(void *data) {
  ...
  kinc_g4_draw_indexed_vertices_instanced(INST_COUNT);
...
int kickstart(int argc, char **argv) {
  ...
  kinc_g4_vertex_structure_add(&structure_inst, "off",
                               KINC_G4_VERTEX_DATA_F32_3X);
  kinc_g4_vertex_structure_add(&structure_inst, "rot",
                               KINC_G4_VERTEX_DATA_F32_3X);
  ...
  kinc_g4_vertex_buffer_init(&vertices_inst, INST_COUNT, &structure_inst,
                             KINC_G4_USAGE_STATIC, 1);
  {
    ...
    for (int inst = 0; inst < INST_COUNT; ++inst) {
      ...
      v[i++] = (float)kinc_random_get_in(-400, 400) / 10.0f;
      v[i++] = (float)kinc_random_get_in(-400, 400) / 10.0f;
      v[i++] = (float)kinc_random_get_in(-314, 314) / 100.0f;
      v[i++] = (float)kinc_random_get_in(-314, 314) / 100.0f;
      v[i++] = (float)kinc_random_get_in(-314, 314) / 100.0f;
    }
